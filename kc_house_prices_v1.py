...
metrics_per_zipcode = data[['price', 'bedrooms', 'bathrooms', 'm2_living', 'm2_lot', 'price_m2', 'zipcode']].groupby(
...
with attribute_filters:
        f_bedrooms_analysis = attribute_filters.slider('Max number of bedrooms', min_bedrooms, max_bedrooms, (min_bedrooms, max_bedrooms))
        f_bathrooms_analysis = attribute_filters.slider('Max number of bathrooms', min_bathrooms, max_bathrooms, (min_bathrooms, max_bathrooms))
        f_floors_analysis = attribute_filters.slider('Max number of floors', min_floors, max_floors, (min_floors, max_floors))

        wfa1, wfa2 = attribute_filters.columns((1, 1))

        wfa_yes = wfa1.checkbox('Has Water View', True)
        wfa_no = wfa2.checkbox('No Water View', True)

        if wfa_yes & wfa_no:
            f_waterfront_analysis = [0, 1]
        elif wf_yes and not wf_no:
            f_waterfront_analysis = [1]
        else:
            f_waterfront_analysis = [0]

temporal_filters = st.sidebar.expander(label='Temporal Analyis Filters')
with temporal_filters:

    f_year_built = temporal_filters.slider( 'Select Max Year Built', min_year_built, max_year_built, 
                                                max_year_built )
    ...
    f_date = temporal_filters.slider( 'Select date Date Range:', min_date, max_date, (min_date, max_date ))
    f_price = temporal_filters.slider( 'Select Price Range:', min_price, max_price, (min_price, max_price))
...
st.markdown("""---""")
st.markdown("<h3 style='text-align: center;'>Temporal Analysis of Prices</h3>", unsafe_allow_html=True)
